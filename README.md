...
**GOOG 12h OHLCV stocks historical data** — ultra high-quality 12h OHLCV for **Alphabet**. Clean `time, open, high, low, close, volume` CSV for backtesting, algorithmic trading and quantitative research.
...
> **Sample on GitHub** · `GOOG_12h.csv` (142 rows, `2026-02-09` -> `2026-09-01`, 8.52 KB). **Full archive on [getdata.finance](https://getdata.finance/datasets/goog)** — **3,113** `12h` rows (full `1m`: 635,127), **11 timeframes**, `2011-05-09` -> `2026-09-01`.
...
| 12h rows | 142 | **3,113** |
| Size | 8.52 KB | full ZIP on [getdata.finance](https://getdata.finance/datasets/goog) |
...
df = pd.read_csv('GOOG_12h.csv', parse_dates=['time'])
